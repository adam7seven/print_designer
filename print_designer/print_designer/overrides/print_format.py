...
class PDPrintFormat(PrintFormat):
    ...
    def write_document_file(self):
        ...
        # create folder
        folder = self.create_folder(doc.doc_type, doc.id)
    ...
    def create_folder(self, dt, dn):
        ...
        folder = os.path.join(
            frappe.get_app_path(self.print_designer_template_app),
    ...
    def export_preview(self, folder, fname):
        if self.print_designer_preview_img:
            ...
            print(f"Wrote preview file for {self.doctype} {self.id} at {target_path}")
            # write the data file
            path = os.path.join(folder, f"print_designer-{fname}-preview.json")
            with open(path, "w+") as json_file:
                json_file.write(frappe.as_json(file_export))
            print(f"Wrote document file for {file.doctype} {file.id} at {path}")
