...
def render_user_text_withdoc(string, doctype, docid=None, row=None, send_to_jinja=None):
    if not row:
        row = {}
    ...
    doc = frappe.get_cached_doc(doctype, docid)
...
@frappe.whitelist(allow_guest=False)
def get_data_from_main_template(string, doctype, docid=None, settings=None):
    if not settings:
        settings = {}
