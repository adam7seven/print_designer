...
def pdf_header_footer_html(soup, head, content, styles, html_id, css):
    if soup.find(id="__print_designer"):
        ...
        try:
            ...
            frappe.throw(
                msg=f"Something went wrong ( Error ) : If you don't know what just happened, and wish to file a ticket or issue on github, please copy the error from <b>Error Log {error.id}</b> or ask Administrator.",
                exc=e,
            )
    else:
...
def pdf_body_html(print_format, jenv, args, template):
    if (
        ...
        print_format_id = hashlib.md5(
            print_format.id.encode(), usedforsecurity=False
        ).hexdigest()
        add_data_to_monitor(
            print_designer=print_format_id, print_designer_action="download_pdf"
        )
        ...
        try:
            ...
            error = log_error(
                ...
                reference_id=print_format.id,
            )
            if frappe.conf.developer_mode:
                return f"<h1><b>Something went wrong while rendering the print format.</b> <hr/> If you don't know what just happened, and wish to file a ticket or issue on Github <hr /> Please copy the error from <code>Error Log {error.id}</code> or ask Administrator.<hr /><h3>Error rendering print format: {error.reference_id}</h3><h4>{error.method}</h4><pre>{html.escape(error.error)}</pre>"
            else:
                return f"<h1><b>Something went wrong while rendering the print format.</b> <hr/> If you don't know what just happened, and wish to file a ticket or issue on Github <hr /> Please copy the error from <code>Error Log {error.id}</code> or ask Administrator.</h1>"
