...
def get_preview_image_folder_path(print_format):
    ...
    return os.path.join(
        frappe.get_app_path(print_format.print_designer_template_app),
        os.path.join(pd_folder[0], app),
    )


def update_preview_img(file):
    print_format = frappe.get_doc(file.attached_to_doctype, file.attached_to_id)
...
def install_default_formats(app, filter_by="", load_pd_formats=True):
    if load_pd_formats:
        ...
        install_default_formats(
            app="print_designer", filter_by=app, load_pd_formats=False
        )
    ...
    print_formats = [f for f in print_formats if not f.id.endswith("-preview.json")]
