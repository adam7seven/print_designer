...
def element(element):
    print(
        "*" * 80, "-" * 32 + " Element Object " + "-" * 32, element, "*" * 80, sep="\n"
    )
...
def dynamic_content_style(style):
    print(
        "*" * 80,
        "-" * 25 + " Dynamic Content Style Object " + "-" * 25,
        style,
        "*" * 80,
        sep="\n",
    )
    ...
    for pf in print_formats:
        ...
        if update_print_json and not is_older_schema(
            ...
            if print_json.get("header", None):
                print_json["header"] = patch_elements(
                    print_json["header"], callbacks, types
                )
            if print_json.get("body", None):
                print_json["body"] = patch_elements(
                    print_json["body"], callbacks, types
                )
            if print_json.get("footer", None):
                print_json["footer"] = patch_elements(
                    print_json["footer"], callbacks, types
                )
